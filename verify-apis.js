...
// 验证API函数
async function verifyAPI(name, api) {
    try {
        ...
        if (response.ok) {
            ...
            if (data && (data.list || data.data)) {
                ...
                console.log(`❌ ${name} (${api}) - 无数据`);
                return false;
            }
        } else {
            console.log(`❌ ${name} (${api}) - ${response.status}`);
            return false;
        }
    } catch (error) {
        console.log(`❌ ${name} (${api}) - ${error.message}`);
        return false;
    }
}
...
async function verifyAllAPIs() {
    ...
    for (const [key, site] of Object.entries(CUSTOMER_SITES)) {
        const success = await verifyAPI(site.name, site.api);
        if (success) {
            results.success.push(site);
        } else {
            results.failed.push(site);
        }
    }
    
    console.log('\n验证完成!');
    console.log(`\n✅ 可用API数量: ${results.success.length}`);
    console.log(`❌ 不可用API数量: ${results.failed.length}`);
